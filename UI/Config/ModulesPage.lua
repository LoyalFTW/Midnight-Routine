...
function Config.BuildModulesPage(ctx)
    ...
    local function SectionLabel(text) yOff = OptionsSectionLabel(body, yOff, text, 8, cfgFs) end
        ...
        local dragHint = ns.AcquireFontString(body, "dragHint", "OVERLAY")
    ...
    local dragGhost = ns.AcquireFrame(body, "dragGhost", "Frame", "BackdropTemplate")
    ...
    dragGhost:Hide()
    local dragGhostLbl = ns.AcquireFontString(dragGhost, "label", "OVERLAY")
    ...
    local dragLine = ns.AcquireFrame(body, "dragLine", "Frame")
    dragLine:SetHeight(2)
    dragLine:SetFrameStrata("TOOLTIP")
    dragLine:Hide()
    local dragLineTex = ns.AcquireTexture(dragLine, "fill", "OVERLAY")
    ...
    local function BuildPatchHeader(patchKey, modKey)
        ...
        local patchFr = ns.AcquireFrame(body, "patchHeader", "Frame", "BackdropTemplate")
        ...
        local cb = ns.AcquireFrame(patchFr, "enable", "CheckButton", "UICheckButtonTemplate")
        cb:SetSize(20, 20)
        ...
        local lbl = ns.AcquireFontString(patchFr, "label", "OVERLAY")
    ...
    local function BuildRowPatchHeader(patchKey, modKey)
        ...
        local patchFr = ns.AcquireFrame(body, "rowPatchHeader", "Frame", "BackdropTemplate")
    ...
    local function BuildRowGroupHeader(modKey, groupRows, label)
        ...
        local ROW_H = moduleRowH
        local groupFr = ns.AcquireFrame(body, "rowGroupHeader", "Frame", "BackdropTemplate")
        ...
        local cb = ns.AcquireFrame(groupFr, "enable", "CheckButton", "UICheckButtonTemplate")
        cb:SetSize(18, 18)
        ...
        local lbl = ns.AcquireFontString(groupFr, "label", "OVERLAY")
        lbl:SetFont(ns.FONT_HEADERS, moduleRowFs, GetFontFlags())
        lbl:SetPoint("LEFT", cb, "RIGHT", 1, 0)
    ...
    local function BuildCustomTaskGroupHeader(modKey, resetType, configGroup)
        ...
        local ROW_H = moduleHeaderH
        local groupFr = ns.AcquireFrame(body, "customTaskGroupHeader", "Frame", "BackdropTemplate")
        groupFr:SetPoint("TOPLEFT", body, "TOPLEFT", 18, yOff)
        groupFr:SetSize(contentW - 20, ROW_H)
        groupFr:SetBackdrop(MakeBackdrop())
        ...
        local checkbox = ns.AcquireFrame(groupFr, "enable", "CheckButton", "UICheckButtonTemplate")
        ...
        local expandBtn = ns.AcquireFrame(groupFr, "expand", "Button", "BackdropTemplate")
        ...
        local expandLbl = ns.AcquireFontString(expandBtn, "label", "OVERLAY")
        ...
        local hideBtn = ns.AcquireFrame(groupFr, "hideComplete", "Button", "BackdropTemplate")
        hideBtn:SetSize(16, 16)
        hideBtn:SetPoint("RIGHT", expandBtn, "LEFT", -2, 0)
        hideBtn:SetBackdrop(MakeBackdrop())
        local hideLbl = ns.AcquireFontString(hideBtn, "label", "OVERLAY")
        ...
        lbl = ns.AcquireFontString(groupFr, "label", "OVERLAY")
    ...
    local function BuildProfessionGroupHeader()
        ...
        local headerFr = ns.AcquireFrame(body, "professionGroupHeader", "Frame", "BackdropTemplate")
        ...
        local badge = ns.AcquireFrame(headerFr, "badge", "Frame", "BackdropTemplate")
        badge:SetSize(28, 24)
        ...
        local badgeText = ns.AcquireFontString(badge, "label", "OVERLAY")
        ...
        local arrowBtn = ns.AcquireFrame(headerFr, "arrow", "Button", "BackdropTemplate")
        ...
        local arrowLbl = ns.AcquireFontString(arrowBtn, "label", "OVERLAY")
        ...
        local lbl = ns.AcquireFontString(headerFr, "label", "OVERLAY")
        lbl:SetFont(ns.FONT_HEADERS, moduleHeaderFs, GetFontFlags())
        ...
        local sub = ns.AcquireFontString(headerFr, "sublabel", "OVERLAY")
        ...
        for _, expansion in ipairs(expansions) do
            if expansion.key ~= "midnight" then
                ...
                if #learned > 0 then
                    ...
                    local expFr = ns.AcquireFrame(body, "legacyExpansionHeader", "Frame", "BackdropTemplate")
                    ...
                    local expLbl = ns.AcquireFontString(expFr, "label", "OVERLAY")
                    ...
                    for _, profession in ipairs(learned) do
                        ...
                        local rowFr = ns.AcquireFrame(body, "legacyProfessionRow", "Frame", "BackdropTemplate")
                        ...
                        local enableBtn = ns.AcquireFrame(rowFr, "enable", "CheckButton", "UICheckButtonTemplate")
                        ...
                        local expandBtn = ns.AcquireFrame(rowFr, "expand", "Button", "BackdropTemplate")
                        expandBtn:SetSize(16, 16)
                        ...
                        local lbl = ns.AcquireFontString(rowFr, "label", "OVERLAY")
                        lbl:SetFont(ns.FONT_ROWS, moduleRowFs, GetFontFlags())
                        ...
                        if isExp then
                            for _, section in ipairs(profession.sections or {}) do
                                if #section.entries > 0 then
                                    ...
                                    local groupFr = ns.AcquireFrame(body, "legacyProfessionSection", "Frame", "BackdropTemplate")
                                    ...
                                    local sectionLbl = ns.AcquireFontString(groupFr, "label", "OVERLAY")
    ...
    local function BuildStoryGroupHeader()
        ...
        local ROW_H = moduleHeaderH
        local headerFr = ns.AcquireFrame(body, "storyGroupHeader", "Frame", "BackdropTemplate")
        headerFr:SetPoint("TOPLEFT", body, "TOPLEFT", 4, yOff)
        headerFr:SetSize(contentW, ROW_H)
        headerFr:SetBackdrop(MakeBackdrop())
        ...
        local cb = ns.AcquireFrame(headerFr, "enable", "CheckButton", "UICheckButtonTemplate")
        cb:SetSize(20, 20)
    ...
    local function BuildProfessionExpansionHeader(expansionKey)
        ...
        local frame = ns.AcquireFrame(body, "professionExpansionHeader", "Button", "BackdropTemplate")
        ...
        local label = ns.AcquireFontString(frame, "label", "OVERLAY")
        ...
        local arrow = ns.AcquireFontString(frame, "arrow", "OVERLAY")
    ...
    local function RenderModuleRows(mod, moduleAvailable)
        local key = mod.key
        local guide = ns.AcquireTexture(body, "moduleGuide", "ARTWORK")
        guide:SetWidth(1)
