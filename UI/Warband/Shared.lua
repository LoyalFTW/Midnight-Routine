...
local function UpdateConcentrationRow(row, entry, contentWidth, mode, compact, rowY, dragSurface)
    ...
    row._label:SetFont(overview and FONT_HEADERS or FONT_ROWS, compact and math.max(8, GetFontSize() - 1) or math.max(9, GetFontSize()), GetFontFlags())
    ...
    row._value:SetFont(FONT_HEADERS, compact and math.max(8, GetFontSize() - 1) or math.max(9, GetFontSize()), GetFontFlags())
    ...
    if overview or not compact then
        ...
        row._gain:SetFont(FONT_ROWS, math.max(8, GetFontSize() - 2), GetFontFlags())
        ...
        row._full:SetFont(FONT_ROWS, math.max(8, GetFontSize() - 2), GetFontFlags())
    ...
    if overview then
        ...
        row._fullAt:SetFont(FONT_ROWS, math.max(8, GetFontSize() - 2), GetFontFlags())
...
local function PopulateConcentrationCards(frame, data, opts)
    ...
    for _, charEntry in ipairs(data or {}) do
        ...
        elseif entries and #entries > 0 then
            ...
            card._name:SetFont(FONT_HEADERS, opts.compact and math.max(9, GetFontSize()) or math.max(10, GetFontSize() + 1), GetFontFlags())
            ...
            card._meta:SetFont(FONT_ROWS, math.max(8, GetFontSize() - 1), GetFontFlags())
