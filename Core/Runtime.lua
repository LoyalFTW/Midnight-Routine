...
function MR:RegisterEvent(event, callback)
    ...
    self._eventController:Register(event, function(firedEvent, ...)
        ...
        -- Handlers are called independently: a handler that errors must not stop
        -- the ones after it, which is what separate frames used to guarantee.
        -- CallErrorHandler runs before the stack unwinds and reports from the
        -- function that threw, so a bug report still carries the full traceback.
        for i = 1, #handlers do
